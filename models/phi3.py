...
本文件实现了Microsoft Phi-3系列模型的高效推理功能，支持ShadowKV稀疏注意力优化。
...
2. 优化策略：
   ...
   - 高效的KV缓存管理：支持超长上下文推理（最大128K tokens）
   - 内存优化：逐层加载权重，及时释放内存
   - GPU加速：所有计算在GPU上进行
...
主要组件：
- Phi3Layer: 单个Transformer层的权重容器
- Phi3: 主要的模型推理类，继承自基础LLM类
...
class Phi3Layer:
    """
    保存单层Phi-3 Transformer所需的权重参数
    
    该类封装了Phi-3模型单个Transformer层的所有权重参数，包括：
    - 自注意力机制的融合QKV投影权重
    - 注意力输出投影权重
    - 前馈网络的融合gate_up投影和下投影权重
    - 层归一化的权重和方差epsilon参数
    
    与Qwen2不同，Phi-3使用融合的QKV投影（类似Llama），没有偏置项。
    """
    ...
    def __init__(self, layer_idx) -> None:
        ...
        """
        # 自注意力机制权重（融合的QKV投影）
        self.wqkv :torch.Tensor = None    # 融合的Query、Key、Value投影权重
        self.wo :torch.Tensor = None      # 注意力输出投影权重

        # 前馈网络权重（融合的gate_up投影）
        self.gate_up_proj :torch.Tensor = None  # 融合的门控和上投影权重
    ...
    def init_parameters(self, hf_layer):
        ...
        Args:
            hf_layer: HuggingFace的Phi-3解码器层
        """
        # 提取融合的QKV投影权重
        self.wqkv :torch.Tensor= hf_layer.self_attn.qkv_proj.weight.detach()  # 融合QKV投影权重
        self.wo :torch.Tensor= hf_layer.self_attn.o_proj.weight.detach()     # 输出投影权重

        # 提取前馈网络权重（融合gate_up）
        self.gate_up_proj = hf_layer.mlp.gate_up_proj.weight.detach()  # 融合的门控和上投影权重
...
class Phi3(LLM):
    ...
    继承自基础LLM类，实现了Microsoft Phi-3系列模型的高效推理功能。
    支持多种优化策略：
    - 标准全注意力模式
    - ShadowKV稀疏注意力模式
    - 高效的KV缓存管理
    ...
    def __init__(self,
        ...
        Args:
            model_name (str): HuggingFace模型名称，默认为"microsoft/Phi-3-mini-128k-instruct"
        ...
        # 设置Phi-3专用模板
        self.ctx_template = Templates['phi']        # 上下文模板
        self.chat_template = Chat_Templates['phi']  # 对话模板
    ...
    def _set_cos_sin_cache(self, hf_model):
        ...
        通过调用HuggingFace模型的RoPE模块来预计算所有位置的cos和sin值。
        
        Args:
            hf_model: HuggingFace的Phi-3模型
    ...
    def pre_attention_compute(
        ...
        query_states = qkv[..., :query_pos]  # Query部分
        key_states = qkv[..., query_pos : query_pos + num_key_value_heads * head_dim]  # Key部分
        value_states = qkv[..., query_pos + num_key_value_heads * head_dim :]  # Value部分
    ...
    def post_attention_compute(
        ...
        # SwiGLU前馈网络（使用融合gate_up投影）
        up_states = F.linear(hidden_states, buffer.gate_up_proj)  # 融合的gate和up投影
        gate_proj, up = up_states.chunk(2, dim=-1)               # 分离gate和up部分
        gate = F.silu(gate_proj)                                 # SiLU激活函数
